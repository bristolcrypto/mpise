...
Note that the addend of `maccfrlu` is L, whereas the addend of `maccfrhu` H. Assuming that the number-representation radix has been chosen properly, there are two carry propagation from L to H and there should also be no overflow (i.e., no carry-out) of H.
...
The highly-efficient reduced-radix implementation using only 2 instructions does not work anymore since each of the accu rehisters can overflow. Thus, we have to manually do a carry propagation from L to M and from M to H.

  ```
  .macro MUL_ADD_FR_ND L, M, H, TL, TM, A, B
