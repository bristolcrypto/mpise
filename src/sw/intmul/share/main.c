...
#if THRESHOLD_DEBUG
#define MAX_LIMB_ITERS 20
unsigned long long rdtsc_debug[MAX_LIMB_ITERS][TRIALS];
#endif
...
int main( int argc, char* argv[] ) {
  ...
  #if THRESHOLD_DEBUG
  unsigned int outer_loop_count=0;
  unsigned int outer_loop_count2=0;
  for( int j = 0; j < MAX_LIMB_ITERS; j++ ) {
  ...
  for( int j = MIN_LIMBS; j <= MAX_LIMBS; j += LIMB_STEP ) {
    ...
    for( int i = 0; i < TRIALS; i++ ) {
      ...
      #if THRESHOLD_DEBUG
      rdtsc_debug[outer_loop_count][trials] = rdtsc_r;
      #endif
      trials    += 1;
    }
  ...
  for( int j = MIN_LIMBS; j <= MAX_LIMBS; j += LIMB_STEP ) {
    ...
    #endif
    #if   defined( MPISE_RADIX_FULL    )
    printf("! %d, %s, %d, %d, %d, %d, %d, %d\n", LIMBBITS, TYPE, MPISE_XLEN, MPISE_DESTRUCTIVE, MPISE_STATELESS, j, j * MPISE_XLEN, TRIALS);
    #elif defined( MPISE_RADIX_REDUCED )
    printf("! %d, %s, %d, %d, %d, %d, %d, %d\n", LIMBBITS, TYPE, MPISE_XLEN, MPISE_DESTRUCTIVE, MPISE_STATELESS, j, j * LIMBBITS,   TRIALS);
    #endif
    for (int t=0; t < TRIALS; t++) {
	if ( t % 100 == 0 ) {
            printf("\n! @ ");
	}
        printf(" %lld", rdtsc_debug[outer_loop_count2][t] );
    }
    printf("\n");
    outer_loop_count2++;
  } // end threshold-debug-data loop
  // Check that we printed the same amount of data as we had recorded.
  if (outer_loop_count2 != outer_loop_count) {
      printf("\nWrong number of iterations processed.\n");
  }
  #endif //end THRESHOLD_DEBUG
  return 0;
}
