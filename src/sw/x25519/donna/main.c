...
// Wrappers for donna_c64 scalar-mul functions
extern int crypto_scalarmult(uint8_t *mypublic, const uint8_t *secret, \
  const uint8_t *basepoint);
extern int crypto_scalarmult_base(unsigned char *q, const unsigned char *n);

#define mon_mul_varbase(rf, kf, xf) \
  crypto_scalarmult((uint8_t *) (rf), (const uint8_t *) (kf), (uint8_t *) (xf)) 
