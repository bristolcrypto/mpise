...
__attribute__((noinline)) void modadd(const spint *a, const spint *b, spint *n) {
  n[0] = a[0] + b[0];
  n[1] = a[1] + b[1];
  n[2] = a[2] + b[2];
  n[3] = a[3] + b[3];
...
__attribute__((noinline)) void modsub(const spint *a, const spint *b, spint *n) {
  n[0] = a[0] - b[0];
  n[1] = a[1] - b[1];
  n[2] = a[2] - b[2];
  n[3] = a[3] - b[3];
...
__attribute__((noinline)) void modmli(const spint *a, int b, spint *c) {
  udpint t = 0;
  spint carry;
  spint s;
  spint mask = ((spint)1 << 29u) - (spint)1;
...
__attribute__((noinline)) void modmul(const spint *a, const spint *b, spint *c) {
  dpint t = 0;
  dpint tt;
  spint lo;
  spint hi;
...
__attribute__((noinline)) void modsqr(const spint *a, spint *c) {
  udpint t = 0;
  udpint tt;
  udpint t2;
  spint carry;
...
void X25519_KEY_PAIR_noinline(char *SK,char *PK)
{
    char bu[Nbytes]={};
    bu[0]=GENERATOR;
    rfc7748(SK,bu,PK);
...
int X25519_SHARED_SECRET_noinline(char *SK,char *PK,char *SS)
{
    char ors=0;
    rfc7748(SK,PK,SS);
